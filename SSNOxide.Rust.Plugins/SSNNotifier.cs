...
namespace Oxide.Plugins
{
    ...
    class SSNNotifier : RustPlugin
    {
        ...
        class ConfigData
        {
            ...
            public Dictionary<ulong, String> CustomPlayerNames = new Dictionary<ulong, String>();
        ...
        public string m_host = "survival-servers-network.xyz";
        ...
        void LoadConfig()
        {
            try
            {
                ...
                if (m_configData.CustomPlayerNames == null)
                {
                    m_configData.CustomPlayerNames = new Dictionary<ulong, string>();
                }
        ...
        void InsertDefaultMessages()
        {
            ...
            InsertDefaultMessage("player_name_was_changed", "Name of player <color=cyan>%player_name</color> was changed to <color=cyan>%custom_player_name</color>.");
            InsertDefaultMessage("player_name_was_reseted", "Name of player <color=cyan>%custom_player_name</color> was reseted to <color=cyan>%player_name</color>.");
            InsertDefaultMessage("custom_player_name_not_found", "Custom player name not found for player <color=cyan>%player_name</color>.");
            InsertDefaultMessage("custom_player_names_is_empty", "Custom player names is empty.");
            InsertDefaultMessage("custom_player_names", "Custom player names:");
        ...
        object CanClientLogin(Network.Connection connection)
        {
            ...
            {
                string playerName = RealPlayerName(userID);
        ...
        void OnPlayerInit(BasePlayer player)
        {
            ...
            if (m_configData.CustomPlayerNames.ContainsKey(player.userID))
            {
                player.displayName = m_configData.CustomPlayerNames[player.userID];
        ...
        void cmdBan(BasePlayer player, string command, string[] args)
        {
            ...
            else
            {
                ...
                ConsoleSystem.Run(ConsoleSystem.Option.Server.Quiet(), "banid", userId, RealPlayerName(userId), reason);
        ...
        void cmdChatBans(BasePlayer player, string command, string[] args)
        {
            ...
            {
                string playerName = CustomOrRealPlayerName(userID);
        ...
        void cmdChatMute(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length < 2)
            {
                if (player != null) player.ChatMessage(m_configData.Messages["invalid_arguments"]);
                return;
            }
            ...
            if (userID == 0)
            {
                if (player != null) player.ChatMessage(m_configData.Messages["player_not_found"]);
                return;
            }
        ...
        {
            if (arg.cmd.Name == "mute")
            {
                if (arg.IsRcon || arg.IsAdmin)
                {
                    cmdChatMute(null, arg.cmd.Name, arg.Args);
                }
            }
            return null;
        }
        ...
        [ChatCommand("set_player_name")]
        void cmdChatSetPlayerName(BasePlayer player, string command, string[] args)
        {
            if (player.net.connection.authLevel == 0 && !permission.UserHasPermission(player.userID.ToString(), "SSNNotifier.player_names"))
            {
                player.ChatMessage(m_configData.Messages["have_not_permission"]);
                return;
            }
            ...
            m_configData.CustomPlayerNames[userID] = name;
            SaveConfig();

            BasePlayer targetPlayer = getPlayer(userID);
            if (targetPlayer != null)
            {
                targetPlayer.displayName = name;
                targetPlayer.SendFullSnapshot();
            }
            PrintToChat(m_configData.Messages["player_name_was_changed"].Replace("%player_name", RealPlayerName(userID)).Replace("%custom_player_name", name));
        }

        //

        [ChatCommand("clear_player_name")]
        void cmdChatClearPlayerName(BasePlayer player, string command, string[] args)
        {
            if (player.net.connection.authLevel == 0 && !permission.UserHasPermission(player.userID.ToString(), "SSNNotifier.player_names"))
            {
                player.ChatMessage(m_configData.Messages["have_not_permission"]);
            ...
            if (!m_configData.CustomPlayerNames.ContainsKey(userID))
            {
                player.ChatMessage(m_configData.Messages["custom_player_name_not_found"].Replace("%player_name", RealPlayerName(userID)));
                return;
            }

            PrintToChat(m_configData.Messages["player_name_was_reseted"].Replace("%player_name", RealPlayerName(userID)).Replace("%custom_player_name", CustomOrRealPlayerName(userID)));

            m_configData.CustomPlayerNames.Remove(userID);
            SaveConfig();

            BasePlayer targetPlayer = getPlayer(userID);
            if (targetPlayer != null)
            {
                targetPlayer.displayName = RealPlayerName(userID);
                targetPlayer.SendFullSnapshot();
            }
        }

        [ChatCommand("custom_player_names")]
        void cmdChatCustomPlayerNames(BasePlayer player, string command, string[] args)
        {
            List<ulong> contextPlayers = new List<ulong>();
            if (m_configData.CustomPlayerNames.Count == 0)
            {
                player.ChatMessage(m_configData.Messages["custom_player_names_is_empty"]);
            }
            else
            {
                player.ChatMessage(m_configData.Messages["custom_player_names"]);
                int i = 0;
                foreach (ulong userID in m_configData.CustomPlayerNames.Keys)
                {
                    contextPlayers.Add(userID);
                    player.ChatMessage(m_configData.Messages["players_line"].Replace("%number", (++i).ToString()).Replace("%player", RealPlayerName(userID)) + " (" + m_configData.CustomPlayerNames[userID] + ")");
                }
            }
            SetContextPlayers(player.userID, contextPlayers);
        }
        ...
        BasePlayer getPlayer(ulong userID)
        {
            foreach (BasePlayer targetPlayer in BasePlayer.activePlayerList)
            {
                if (targetPlayer.userID == userID) return targetPlayer;
            }
        ...
        private string RealPlayerName(ulong userID)
        {
        ...
        private string CustomOrRealPlayerName(ulong userID)
        {
            BasePlayer player = getPlayer(userID);
            if (player != null)
            {
                return player.displayName;
            }
            return RealPlayerName(userID);
        }
        ...
        private void NotifyPlayerKill(BasePlayer victimPlayer, BasePlayer killerPlayer, ItemDefinition itemDefinition, double distance, bool isHeadshot, bool isSleeping)
        {
            ...
            jsonMurder.victimPlayer = new JsonPlayer(victimPlayer.userID, RealPlayerName(victimPlayer.userID));
            jsonMurder.killerPlayer = new JsonPlayer(killerPlayer.userID, RealPlayerName(killerPlayer.userID));
        ...
        {
            SendWebRequest("player/chat_message", JsonConvert.SerializeObject(new JsonPlayerChatMessage(player, RealPlayerName(player.userID), messageText)), (code, response) => ReceiveWebResponse(code, response));
        }

        private void NotifyPlayerBan(ulong steamid, string reason)
        {
            SendWebRequest("player/ban", JsonConvert.SerializeObject(new JsonPlayerBan(steamid, RealPlayerName(steamid), reason)), (code, response) => ReceiveWebResponse(code, response));
        }

        private void NotifyPlayerMute(ulong steamid, string reason)
        {
            SendWebRequest("player/mute", JsonConvert.SerializeObject(new JsonPlayerMute(steamid, RealPlayerName(steamid), reason)), (code, response) => ReceiveWebResponse(code, response));
        }
        ...
        private void NotifyServerStatus()
        {
            ...
            {
                jsonServerStatus.onlinePlayers[i] = new JsonOnlinePlayer(BasePlayer.activePlayerList[i].userID, RealPlayerName(BasePlayer.activePlayerList[i].userID), BasePlayer.activePlayerList[i].net.connection.ipaddress.Split(':')[0]);
            }
